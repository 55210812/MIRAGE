...
def compute_claims(data):
    ...
    if autoais_model is None:
        ...
        autoais_model = AutoModelForSeq2SeqLM.from_pretrained(AUTOAIS_MODEL, torch_dtype=torch.bfloat16, max_memory=get_max_memory(), device_map="auto", cache_dir='/projects/0/prjs0888/plms/')
...
def compute_autoais(fname, data,
                    decontext=False,
                    concat=False,
                    qampari=False,
    ...
    return {
        ...
        "citation_f1": 200 * np.mean(ais_scores_prec) * np.mean(ais_scores) / (np.mean(ais_scores) + np.mean(ais_scores_prec)),
    }
...
def main():
    ...
    for i in range(len(data)):
        if "llama" in args.f.lower():
            data[i]['output'] = data[i]['output'].strip()
            for j in range(10):
                r_tmp = "\n" * (10-j)
                data[i]["output"] = data[i]["output"].replace(r_tmp, " ")
        else:
